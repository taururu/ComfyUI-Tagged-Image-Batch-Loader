...
| `save_prefix` | STRING | 保存ノードの `filename_prefix` に渡す文字列（日付フォルダ込み） |
| `filename_body` | STRING | `save_prefix` から日付フォルダ部分を除いた、ファイル名部分のみの文字列 |
...
filename_body  = Name-A_Dress_Dance_20260618-153020
```

`save_prefix` を Video Combine 等の `filename_prefix` に接続すると、ComfyUI の output 配下に次のように保存されます。
...
**`filename_body` の用途:** D2 Send Eagle の `filename_template` のように、フォルダ区切り（`/`）を含む文字列を受け付けないノードに接続する場合はこちらを使います。日付フォルダ分けが不要な出力名にそのまま使えます。
